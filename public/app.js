...
if (state.apiKey || S.get("skipOnboarding", false)) {
  obOverlay.style.display = "none";
  mainApp.style.display = "block";
  initApp();
}
...
function initApp() {
  ...
  const sk = document.getElementById("settingsKey");
  const sd = document.getElementById("settingsDate");
  if (sk) sk.value = state.apiKey || "";
...
function openSettings() {
  document.getElementById("settingsKey").value = state.apiKey || "";
  document.getElementById("settingsDate").value = state.examDate || "";
  document.getElementById("settingsModal").style.display = "flex";
}
function closeSettings() { document.getElementById("settingsModal").style.display = "none"; }
function saveSettings() {
  state.apiKey = document.getElementById("settingsKey").value.trim();
...
function renderPodcast(el) {
  ...
  document.getElementById("genPodcast").addEventListener("click", async () => {
    ...
    if (!state.apiKey) { st.textContent = "Ingen API-nøgle. Gå til ⚙ Indstillinger."; st.className = "status err"; return; }
    ...
    form.append("mode", "podcast"); form.append("apiKey", state.apiKey);
...
function renderVidenhuller(el) {
  ...
  document.getElementById("vhStart").addEventListener("click", async () => {
    ...
    if (!state.apiKey && file) { st.textContent = "Ingen API-nøgle til AI-analyse."; st.className = "status err"; return; }

    if (file && state.apiKey) {
      st.innerHTML = `<div class="pbar"><div class="pbar-fill"></div></div>Analyserer…`;
      const form = new FormData();
      form.append("file", file); form.append("mode", "gaps"); form.append("count", "8"); form.append("apiKey", state.apiKey);
      try {
        const r = await fetch("/api/pdf", { method: "POST", body: form });
        const d = await r.json();
        if (!r.ok || !d.ok) throw new Error(d.error);
...
function renderChat(el, mode = "tutor", context = "") {
  ...
  async function send() {
    ...
    if (!state.apiKey) {
      // Fallback: regelbaseret tutor
      const r = await fetch("/api/tutor", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({message:msg}) });
      const d = await r.json();
    ...
    try {
      const r = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: history, mode, apiKey: state.apiKey, context }),
      });
      const d = await r.json();
...
function renderQuizSetup(el, examMode = false, overrideItems = null) {
  ...
  document.getElementById("qAiStart").addEventListener("click", async () => {
    ...
    if (!state.apiKey) { st.textContent = "Ingen API-nøgle."; st.className = "status err"; return; }
    st.innerHTML = `<div class="pbar"><div class="pbar-fill"></div></div>AI genererer spørgsmål…`;
    const form = new FormData();
    form.append("file", file); form.append("mode", "quiz"); form.append("count", "10"); form.append("apiKey", state.apiKey);
    try {
      const r = await fetch("/api/pdf", { method:"POST", body:form });
      const d = await r.json();
      if (!r.ok || !d.ok) throw new Error(d.error);
...
function renderFlashSetup(el) {
  ...
  document.getElementById("fAiLoad").addEventListener("click", async () => {
    ...
    const form = new FormData();
    form.append("file", file); form.append("mode", "flash"); form.append("count", "12"); form.append("apiKey", state.apiKey);
    try {
      const r = await fetch("/api/pdf", {method:"POST",body:form});
      const d = await r.json();
      if (!r.ok || !d.ok) throw new Error(d.error);
...
function renderMundtlig(el) {
  ...
  document.getElementById("oralSubmit").addEventListener("click", async () => {
    ...
    try {
      const r = await fetch("/api/chat", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ messages, mode:"oral", apiKey:state.apiKey }),
      });
      const d = await r.json();
      if (d.error) throw new Error(d.error);
