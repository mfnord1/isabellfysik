...
const numOptions = (correct, unit = "", spread = 0.4) => {
  ...
  // Byg distraktorer som tal
  const nums = [c];
  let attempts = 0;
  while (nums.length < 4 && attempts < 100) {
    attempts++;
    const f = 1 + (Math.random() * 2 - 1) * spread;
    let v = Math.round(c * f * 100) / 100;
    if (v === c || v <= 0 || nums.includes(v)) continue;
    nums.push(v);
  }
  // Konvertér til strings EN gang — answer og options kan aldrig afvige
  const fmt = (v) => `${v}${unit ? " " + unit : ""}`;
  const correctStr = fmt(c);
  const opts = shuffle(nums.map(fmt));
  // Paranoid check: sikr at correctStr er i opts
  if (!opts.includes(correctStr)) opts[0] = correctStr;
  return { options: opts, answer: correctStr };
};
